      ...
      {sendMode === 'individual' && (
        <div className="relative">
          ...
          {showDropdown && emailSearch.trim() && !targetEmail && (
            <div className="absolute z-20 top-full left-0 right-0 mt-1 bg-admin-surface border border-admin-border rounded-lg shadow-lg max-h-48 overflow-y-auto">
          ...
          {showDropdown && emailSearch.trim() && !targetEmail && (
            <div
              className="absolute z-20 bottom-0 left-0 right-0 bg-admin-surface border border-admin-border rounded-b-lg px-4 py-2 cursor-pointer hover:bg-admin-brand-activeBg"
              onMouseDown={(e) => {
                e.preventDefault();
      ...
      <div className="flex items-center gap-2 mb-4">
        <button
          ...
          className={`flex-1 flex items-center justify-center gap-2 px-3 py-2.5 rounded-lg text-xs font-medium transition-colors ${
            sendMode === 'broadcast' ? 'bg-admin-brand text-white' : 'bg-admin-surface border border-admin-border text-admin-text-secondary'
          }`}
        >
        ...
        <button
          onClick={() => setSendMode('individual')}
          className={`flex-1 flex items-center justify-center gap-2 px-3 py-2.5 rounded-lg text-xs font-medium transition-colors ${
            sendMode === 'individual' ? 'bg-admin-brand text-white' : 'bg-admin-surface border border-admin-border text-admin-text-secondary'
          }`}
        >
        ...
        <div className="relative">
          ...
          {!targetEmail && (
            <input
              ...
              onBlur={() => setTimeout(() => setShowDropdown(false), 150)}
              placeholder="Search by name or email..."
              className="w-full px-4 py-3 bg-admin-surface border border-admin-border rounded-lg text-sm text-admin-text-primary outline-none placeholder:text-admin-text-muted focus:border-admin-brand transition-colors"
            />
          )}
          {showDropdown && emailSearch.trim() && !targetEmail && (
            <div className="absolute z-20 top-full left-0 right-0 mt-1 bg-admin-surface border border-admin-border rounded-lg shadow-lg max-h-48 overflow-y-auto">
          ...
          {showDropdown && emailSearch.trim() && !targetEmail && (
            <div
              className="absolute z-20 bottom-0 left-0 right-0 bg-admin-surface border border-admin-border rounded-b-lg px-4 py-2.5 cursor-pointer hover:bg-admin-brand-activeBg"
              onMouseDown={(e) => {
                e.preventDefault();
                setTargetEmail(emailSearch.trim());
                setEmailSearch('');
...
export default function AddNotificationSection({ onBack, prefillEmail = '', prefillTitle = '', prefillMessage = '', prefillChannels = ['app'] }) {
  ...
  return (
    <>
                  ...
                  <button
                    ...
                    className={`flex-1 flex items-center gap-3 p-4 rounded-xl border-2 transition-all ${
                      selected
                        ...
                        : 'border-admin-border/40 bg-admin-surface hover:border-admin-border'
                    }`}
                  >
      ...
      {/* Mobile */}
      <div className="lg:hidden min-h-screen bg-admin-surface pb-24">
        <div className="sticky top-0 z-10 bg-admin-surface border-b border-admin-border/30">
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 p-3 rounded-xl border-2 transition-all ${
                      selected
                        ? 'border-admin-brand bg-admin-brand/5'
                        : 'border-admin-border/40 bg-admin-surface'
                    }`}
                  >
        ...
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-admin-surface border-t border-admin-border/30">
          <button
            onClick={handleSend}
            disabled={submitting || !title.trim() || !message.trim() || (sendMode === 'individual' && !targetEmail.trim()) || (scheduleEnabled && (!scheduledDate || !scheduledTime))}

    ...
    <>
      ...
      <div className="lg:hidden px-4 py-6 flex flex-col gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          {selectedIds.size > 0 && (
            <>
              ...
              <button onClick={handleDelete} className="flex items-center gap-1.5 px-4 py-2.5 bg-accent-orange/10 text-accent-orange rounded-xl text-xs font-medium">
                <Trash2 className="w-3.5 h-3.5" /> {t('admin.products.selectDelete', { count: selectedIds.size })}
              </button>
            </>
          )}
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => setShowFilterPanel(!showFilterPanel)}
            className={`w-10 h-10 flex items-center justify-center rounded-xl border transition-colors ${showFilterPanel || hasFilters ? 'bg-admin-brand border-admin-brand text-white' : 'bg-surface-elevated border-admin-border text-admin-text-secondary'}`}
          >
            <SlidersHorizontal className="w-4 h-4" />
          </button>
        </div>

  ...
  return (
    <div className={`absolute left-0 top-0 flex h-full w-full flex-col justify-center transition-all duration-700 md:w-[60%] ${
      ...
      <div className="mx-auto w-full max-w-md">
        <h2 className={`font-bold text-black ${inModal ? 'text-3xl' : 'text-4xl'}`}>{t("auth.loginTitle")}</h2>
        <p className={`mt-2 font-semibold text-gray-500 ${inModal ? 'mb-6 text-sm' : 'mb-10'}`}>{t("auth.loginSubtitle")}</p>
        <form onSubmit={onLogin} className={inModal ? 'space-y-3' : 'space-y-6'}>
          <div className="flex flex-col gap-2">
            ...
            <input name="email" type="email" placeholder={t("auth.emailPlaceholder")} required className={`w-full rounded-xl border border-gray-300 text-sm focus:border-[#40B9FF] outline-none ${inModal ? 'p-3' : 'p-4'}`} />
          </div>
          <div className="flex flex-col gap-2">
            <label className="text-sm font-bold text-black">{t("auth.password")}</label>
            ...
            <div className="text-right">
              <button type="button" onClick={onForgot} className="text-xs font-semibold text-[#40B9FF] hover:underline">{t("auth.forgotPassword")}</button>
            </div>
          </div>
          <button type="submit" className={`w-full rounded-xl bg-[#40B9FF] font-bold text-white shadow-lg transition-transform active:scale-95 hover:bg-[#89D4FF] ${inModal ? 'py-3' : 'py-4'}`}>{t("auth.login")}</button>

...
export default function AuthPage({
  ...
  useEffect(() => {
    const handleMessage = event => {
      ...
      if (event.data?.type === 'GOOGLE_AUTH_SUCCESS') {
        ...
        showToast(
          'error',
          getGoogleErrorMessage(event.data.payload),
          toastPosition,
        );
      }
    };
  ...
  const handleRegister = async e => {
    ...
    try {
      ...
      console.log('User profile data after signup', {
        user: userRes.data,
        timestamp: new Date().toISOString(),
      });
      const apiUser = userRes.data?.user || userRes.data;
  ...
  const handleForgotSubmit = async e => {
    ...
    try {
      ...
      showToast(
        'error',
        error.response?.data?.message || t('auth.failedToSendReset'),
      );
    }
  };
  ...
  const handleResetPassword = async (password, confirmPassword) => {
    ...
    try {
      ...
      showToast(
        'error',
        error.response?.data?.message || t('auth.failedToReset'),
      );
    }
  };
  ...
  return (
    <div
      className={`flex ${inModal ? '' : 'min-h-screen'} items-center justify-center ${inModal ? '' : 'bg-surface-elevated p-4'} font-sans`}
    >
      <div
        dir={isArabic ? 'rtl' : 'ltr'}
        className={`relative ${inModal ? 'h-[600px]' : 'h-[750px]'} w-full ${inModal ? '' : 'max-w-6xl'} overflow-hidden rounded-[40px] bg-surface-elevated shadow-2xl`}
      >
        <Login

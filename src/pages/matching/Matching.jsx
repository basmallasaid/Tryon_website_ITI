...
export default function Matching() {
  ...
  const steps = [
    {
      id: 1,
      title: t('matching.step1Title'),
      subtitle: t('matching.step1Subtitle'),
    },
    {
      id: 2,
      title: t('matching.step2Title'),
      subtitle: t('matching.step2Subtitle'),
    },
    {
      id: 3,
      title: t('matching.step3Title'),
      subtitle: t('matching.step3Subtitle'),
    },
  ];
  ...
  const currentStep = showResults
    ...
    : isLoading
      ? 2
      : selectedItemId || galleryFile
        ? 2
        : 1;
  const isReady =
    (itemSource === 'wardrobe' && selectedItemId) ||
    (itemSource === 'gallery' && galleryFile);

  const hasItem = !!selectedItemId || !!galleryFile;
  const selectedTitle = hasItem
    ? t('matching.oneItemSelected')
  ...
  const handleFindMatches = async () => {
    ...
    try {
      const processMatches = raw => {
        ...
        console.log(
          'Wardrobe matches:',
          list.filter(m => m.item?.source === 'wardrobe').length,
        );
        console.log(
          'Store matches:',
          list.filter(m => m.item?.source === 'store').length,
        );
        console.log(
          'Sources:',
          list.map(m => m.item?.source),
        );
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 sm:py-14">
        ...
        <section className="mt-12 sm:mt-16">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 max-w-6xl mx-auto">
            <div
              onClick={() =>
                setItemSource(itemSource === 'wardrobe' ? null : 'wardrobe')
              }
              className={`relative p-6 rounded-2xl cursor-pointer flex items-center justify-between transition-all outline-none w-full h-full ${
                itemSource === 'wardrobe'
                  ? 'bg-info-bg border-2 border-primary'
              ...
              <div className="flex items-center justify-between w-full">
                ...
                <div className="flex items-center gap-3 shrink-0 ml-4 mr-6">
                  <div className="w-24 h-24 bg-info-bg rounded-2xl flex flex-col items-center justify-center border border-dashed border-[var(--border)]">
                    <Shirt className="w-8 h-8 text-info-text" />
                  </div>
                </div>
              </div>
              <div
            ...
            <div
              onClick={() =>
                setItemSource(itemSource === 'gallery' ? null : 'gallery')
              }
              className={`relative p-6 rounded-2xl cursor-pointer flex items-center justify-between transition-all outline-none w-full h-full ${
                itemSource === 'gallery'
                  ? 'bg-info-bg border-2 border-primary'
            ...
            >
              <div className="flex items-center justify-between w-full">
                ...
                <div className="flex items-center gap-3 shrink-0 ml-4 mr-6">
                  <div className="w-24 h-24 bg-info-bg rounded-2xl flex flex-col items-center justify-center border border-dashed border-[var(--border)]">
                    <Grid3x3 className="w-8 h-8 text-info-text" />
                  </div>
                </div>
              </div>
              <div
            ...
            <div className="max-w-6xl mx-auto">
              ...
              {galleryFile && galleryPreview && (
                <div className="flex justify-center">
                  <div className="relative w-44 sm:w-52 rounded-2xl p-3 sm:p-4 bg-info-bg">
                    <div className="relative aspect-[3/4] overflow-hidden rounded-lg bg-surface-elevated">
                      ...
                      <button
                        ...
                        className={`absolute ${isArabic ? 'left-2' : 'right-2'} top-2 flex h-7 w-7 items-center justify-center rounded-full bg-surface-elevated/90 text-text-secondary shadow-md transition-all duration-200 hover:bg-accent-orange hover:text-white hover:scale-110`}
                      >
                        <X className="h-4 w-4" />
                      </button>
                    </div>
          ...
          <section ref={resultRef} className="mt-12 sm:mt-16 space-y-32">
            <div className="max-w-6xl mx-auto">
                  ...
                  {wardrobeMatches.length === 0 ? (
                    ...
                    wardrobeMatches.map((match, index) => {
                      ...
                      return (
                        ...
                        >
                          <div className="relative w-full h-full bg-surface-elevated rounded-[9px] flex flex-col items-center justify-center overflow-hidden">
                            <div
                              className={`absolute top-1 ${isArabic ? 'left-1' : 'right-1'} z-10 bg-secondary text-white text-[9px] font-bold px-[6px] py-[2px] rounded-full`}
                            >
                              {match.score}%
                            </div>
                            <button
                            ...
                            {imgUrl ? (
                              ...
                              <Shirt className="w-[40%] h-auto text-text-disabled" />
                            )}
                            <div className="absolute inset-0 bg-black/40 backdrop-blur-[1px] rounded-[9px] opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center z-20 pointer-events-none">
                              <div
                                ...
                                onClick={e => {
                                  e.stopPropagation();
                                  const itemId =
                                    match.item?._id || match.item?.id;
                                  if (itemId)
                                    navigate('/wardrobe/edit/' + itemId);
                                }}
                              >
                                <ArrowRight
                                  size={22}
                      ...
                      return (
                        ...
                        >
                          <div className="relative w-full h-full bg-surface-elevated rounded-[9px] flex flex-col items-center justify-center overflow-hidden">
                            ...
                            <button
                              onClick={handleFav}
                              className={`absolute top-1 z-30 p-1 rounded-full bg-surface-elevated/80 backdrop-blur-sm shadow-sm hover:scale-110 transition-all cursor-pointer ${isArabic ? 'right-1' : 'left-1'}`}
                            >
                              <Heart
                                size={12}
                                className={
              ...
              <div className="flex justify-center pb-10 mt-16">
                <button
                  onClick={handleReset}
                  className={`inline-flex items-center justify-center gap-2 w-full max-w-md py-4 rounded-xl font-bold text-white bg-secondary hover:opacity-90 hover:scale-105 active:scale-95 transition-all shadow-lg cursor-pointer ${isArabic ? 'flex-row-reverse' : ''}`}
                >
                  <Sparkles className="w-5 h-5" />
                  {t('matching.tryAgain')}
                </button>
